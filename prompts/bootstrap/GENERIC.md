...
6. Keep always-on instructions concise. Do not copy ARCHITECTURE.md, completed-task history, judgment history, usage ledger, execution ledger, retrospectives, stories, or every skill body into permanent context. Expose skills/tools/history lazily where supported.
...
11. Preserve hot-state/cold-history separation: ordinary runs discover current task/checkpoint/index without recursively reading completed tasks/judgments/usage/history/stories.
12. For long campaigns/rescues/migrations where later work reconstruction matters, offer the cold execution ledger from schemas/EXECUTION_RETROSPECTIVE_LEDGER.md and prompts/workflow/RECORD_EXECUTION_BOUNDARY.md. Do not enable it as mandatory ceremony for ordinary small tasks.
13. If the harness lacks a feature, document the limitation and preserve the conceptual lifecycle with files/prompts instead of pretending the feature exists.
14. Before writing, report the adapter plan and conflicts with existing files.
15. After writing, verify that a fresh session can discover:
   ...
   - optional retrospective path when explicitly requested;
   - build/test commands;
   without loading the entire framework into permanent context.
16. Return changed paths and a concise validation receipt. Do not start product work.
```
